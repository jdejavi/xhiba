...
{	
	...
	my $prep = $dbh->prepare('SELECT service_email FROM users WHERE username = ?');
	$prep->execute($name);
	
	my @row = $prep->fetchrow_array;

	if ($row[0] == 0){
		my $prep2 = $dbh->prepare('update users set service_email = 1 WHERE username = ?');
		$prep2->execute($name);
	}else{
		my $prep2 = $dbh->prepare('update users set service_email = 0 WHERE username = ?');
		$prep2->execute($name);
